...
function PropertyCard({ property, href }: PropertyCardProps) {
		...
		<Card className="w-full max-w-75 mb-1 overflow-hidden border-[#E5E5E5] shadow-sm transition-transform duration-200 hover:-translate-y-0.5 hover:shadow-md">
			...
			<CardHeader>
				...
				<div className="flex items-center gap-2">
					...
					<p className="text-[12px] text-[#5D5D5D] font-medium">{property.location}</p>
				</div>
			...
			<CardContent className="flex items-center gap-4">
				<div className="flex items-center px-2">
					...
					<p className="text-[12px] text-[#5D5D5D] font-medium">{property.beds}</p>
				</div>
				<div className="flex items-center gap-2">
					<img src="/images/vector1.svg" alt="area" />
					<p className="text-[12px] text-[#5D5D5D] font-medium">{property.area}</p>
				</div>
				<div className="flex items-center gap-2">
					<img src="/images/vector2.svg" alt="road access" />
					<p className="text-[12px] text-[#5D5D5D] font-medium">{property.roadAccess}</p>
				</div>
			</CardContent>
		</Card>
	);
	...
	return (
		<Link href={href} className="block w-full max-w-75" aria-label={`View details for ${property.title}`}>
			{cardContent}
		</Link>
	);
}
